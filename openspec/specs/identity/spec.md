...
TBD — captures who the user is: the Google Sign-In gate that must be passed before any app view renders, how the intended destination survives sign-in, how the signed-in account is presented and signed out, how the session persists across reload, and how each account forms its own per-user data partition keyed by `userId`.
