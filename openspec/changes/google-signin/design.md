...
- **Layer 2 (use case):** `IdentityUseCase` — `restore()`, `signIn()`, `signOut()`, exposing the current identity — orchestrating the `IdentityProvider` and `SessionStore` ports.
- **Layer 3 (ports):** `IdentityProvider`, `SessionStore` interfaces.
- **Layer 4 (frameworks):** GIS adapter; `SessionStore` adapter; React `IdentityProvider`/`useIdentity` context; the sign-in screen; the shell's account control.
...
- **[jsdom cannot run the GIS popup]** → the use case and gate are tested against the fake `IdentityProvider`; the real popup, session-restore-on-reload, and account switching are in the change's manual verification step per AGENTS.md.
