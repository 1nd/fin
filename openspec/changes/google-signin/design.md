...
- Authenticate in the browser with no server and no redirect URIs; derive a stable `userId` and capture profile.
...
- Retire the placeholder id. (Activating the settings cascade's account-locale tier was a goal here until verification showed Google no longer issues the `locale` claim; the tier is dropped instead — D10.)
...
- A pre-sign-in language switcher is not expected in Phase 1. The sign-in screen follows browser locale, and the Google button self-localizes; any stored override takes over on sign-in, so a mismatch is confined to one pre-auth screen. A switcher is awkward precisely here — preference persistence is keyed by `userId` and none exists yet, so persisting a choice would need a fourth locale source outside the per-user cascade. If real friction appears (e.g. shared-device users whose browser locale differs from their language), open a discussion; nothing in the spec precludes adding one later.

## Decisions

### D1 — Google Identity Services, ID-token flow (not the access-token / auth-code flow)

Phase 1 needs **authentication** — a stable per-account id plus profile (locale was expected too; it turned out Google no longer issues it — D10) — not **authorization** to call Google APIs. Google Identity Services (GIS) "Sign In With Google" issues an **ID token** (a JWT with `sub`, `name`, `email`, `picture` — historically also `locale`, no longer issued) via a popup/embedded flow that uses only whitelisted JavaScript origins and **no redirect URIs** — exactly the no-server constraint. The `sub` claim is the `userId`; the other claims drive display.
...
A layer-2 `IdentityProvider` port exposes `renderInto(container): Promise<void>` (mounts the sign-in affordance once) and `onIdentity(listener): unsubscribe` (delivers one `IdentitySignInResult` per attempt); a layer-4 **GIS adapter** implements it — the only production implementation, since no "local user" adapter exists (D8). The port is shaped as a subscription, not a one-shot `Promise<UserIdentity>`: the rendered affordance (the Google button) stays mounted and can drive an unbounded number of attempts — every click re-invokes GIS's `callback` against the same `initialize()` registration (a rejected token followed by a valid one both land here; a dismissed popup with no token gives the adapter no signal at all — see Risks) — so a Promise that only ever settles once would silently drop every attempt after the first (a retry-after-failure regression caught in review). Tests use an in-memory **mock** implementing the same port: this keeps the seam honest and makes the use case testable in jsdom, which cannot run GIS. Swapping to a Phase-X server-backed session, or another IdP, replaces the adapter and touches nothing above it.

**Cross-platform (Phase Y — iOS/Android):** this port *is* the seam that lets native ports land without reworking identity. The entity, use case, and ports (layers 1–2) port unchanged; the GIS adapter, the `SessionStore` adapter, and the sign-in UI (layer 4) are re-implemented per platform — Android via Credential Manager, iOS via the GoogleSignIn SDK — each delivering the *same* `UserIdentity` from a Google ID token through `onIdentity`. Two invariants keep this cheap and must hold: (a) **the port stays token-neutral** — `onIdentity` delivers a decoded `UserIdentity`, never a GIS/JWT-shaped credential, so no web type leaks into a native adapter; (b) the gate reads identity state, never URLs (see D5), so a URL-less native navigation stack re-expresses the same decision without touching the use case. Combined with `userId = sub` (D3, stable for one account across platforms under a single Google Cloud project), a user signs in on any platform as the same user — no identity rework, only per-platform adapters and config.

**CCA layering:**

- **Layer 1 (entity):** `UserIdentity` (`userId`, `displayName`, `email`, `pictureUrl?`) and the pure rule deriving `userId` from the Google subject.
- **Layer 2 (use case + ports):** `IdentityUseCase` — `restore()`, `renderInto()`, `onIdentity()`, `signOut()`, exposing the current identity — orchestrating the `IdentityProvider` and `SessionStore` ports it owns and depends on (dependency inversion: the port belongs to the layer that calls it, not the adapter ring that implements it).
...
The sign-in screen still needs i18n and theme before any account exists. `PreferencesProvider` is generalized to take `userId: string | null` from the identity context: with `userId === null` it resolves the **browser-locale cascade** and **does not persist** (Settings is unreachable while gated anyway), so the sign-in screen is fully localized/themed through the existing settings machinery — one i18n/theme path, no bootstrap fork.
...
`PreferencesProvider` (`src/settings/PreferencesContext.tsx`) stops hardcoding `LOCAL_PLACEHOLDER_USER_ID`. It reads `userId` from `useIdentity()`; when `userId` changes (sign-in, switch, sign-out) it rebuilds `SettingsUseCase` for that id and reloads that user's overrides. (As first designed it also threaded `accountLocale` into `resolvePreferences` to activate the cascade's dormant top tier; that tier is dropped now that the claim is verified to never arrive — D10.) Because IndexedDB stores are already keyed `[userId, key]`, switching accounts just changes which rows are read; nothing migrates and no cross-account leakage is possible. The existing load-in-flight, persistence-failure, and atomic-remove behaviors are unchanged — only the `userId` source moves.
...
**Corner test (plan-level engineering practice):** identity's entire footprint is the layer-4 adapter + two ports, the root-level gate, and the `userId` input into `PreferencesProvider`. Swapping the IdP, or replacing the local session with a Phase-X server-backed one, is a mechanical change across exactly those points — the `UserIdentity` entity, the use case, the gate's shape, and every layer-1–3 consumer of `userId` stay put. The pick is justified against Fin's eventual multi-user/server shape, not today's single developer.
...
- `sub` is pseudonymous (opaque, not name/email) — the privacy-preferable key. Profile fields (name/email/picture) are captured for display only.
...
- **Least privilege:** only `openid`/`profile`/`email` (the `locale` claim was expected under these scopes too, but Google no longer issues it — D10); Drive is deferred to `(D)`.
...
### D10 — The account-locale cascade tier is dropped (decided post-verification)

Manual verification (task 9.2, 2026-07-19) confirmed the `locale` claim is gone from Google's ID tokens: a test account with its Google-side language set to Indonesian, signing in fresh (no stored override) on an `en-US` browser, persisted a session with no `locale` field at all, and defaults fell through to browser locale (`numberFormat` resolved to the `en` cascade's period-decimal `1,234.56`, not `id`'s comma-decimal) — the cascade silently skipped the account tier because the claim genuinely never arrived.

The tier's whole purpose was a better-than-browser default at **first render, before any preference exists** — and no plausible source can deliver an account locale at that moment. The People API (`personFields=locales`) needs an OAuth2 access token, and requesting one at sign-in would break this change's identity-only least-privilege posture; the access token Fin will eventually hold (`(D)`, Drive scope) arrives long after first render; a Phase-X server has the same data problem, not a better one; native SDKs (Phase Y) supply *device* locale — the browser-locale tier's analogue — not account language. So rather than ship an inert tier wrapped in caveats, the tier is removed: `UserIdentity` drops `locale`, the GIS decoder stops reading the claim, `resolvePreferences`/`getEffectivePreferences` collapse to browser locale → fallback, and `PreferencesProvider` stops threading `accountLocale` (tasks §10).

Corner test: the removal is as confined as the addition was — the entity field, the decoder, the pure resolver's account-locale parameter, the provider threading, and the spec scenarios. Persisted sessions that still carry a `locale` field restore fine (the session validator ignores unknown fields) — no migration, consistent with D8's stance. Reintroducing the tier is not expected; if a source that can deliver an account locale at first sign-in ever appears, open a discussion — the seam is mechanical to reopen (an optional entity field plus a resolver parameter).

## Risks / Trade-offs

- **[GIS script is an external dependency; if `accounts.google.com` is blocked, first sign-in fails — and the whole app is gated behind it]** → restore is local (D4), so already-signed-in users keep working offline; only first sign-in / switching needs Google. First-sign-in failure renders a localized notice with a retry control, not a crash or a dead end — this case never got as far as rendering the Google button, so (unlike a rejected token, where the real button is already there and already retryable) there is nothing else on screen to click without it. This is inherent to "sign-in required" and acceptable under the Phase-1 convenience posture.
...
- **[No account-locale defaults — the cascade's top tier was dropped (D10) after verification showed Google no longer issues the `locale` claim]** → browser locale still gives every new user a reasonable default and Settings overrides are one click away; if the missing tier ever becomes real friction, revisit via D10's reintroduction condition.
...
- **[`google.accounts.id` (the ID-token API this adapter uses) has no `error_callback` — that field exists only on the unrelated OAuth2 token client — so GIS gives the adapter no signal when a user closes the button's popup without picking an account]** → accepted: the sign-in screen is simply left as-is (no notice), and the button remains clickable for another attempt; this is a hard API limitation, not an implementation gap, and is reflected in the spec as its own scenario (a dismissed prompt vs. a failed one) rather than one scenario promising a notice in both cases.
