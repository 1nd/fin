...
- [x] 1.4 Add `// CCA: <n>` layer tag comments (1 Entities, 2 Use Cases, 3 Interface Adapters, 4 Frameworks & Drivers) as the first line of every existing `src/` file, and adopt the convention for all new files (design Decision 10)
- [x] 1.5 Set up the test runner and an `npm test` script: single Jest runner via jest-expo + @testing-library/react-native (design Decision 11)
- [x] 1.6 Add a pre-commit hook via husky + lint-staged that runs Prettier and ESLint (with `--fix`) on staged files only — no typecheck or tests in the hook (those stay in manual scripts/CI)
- [ ] 1.7 Add GitHub Actions CI workflow running lint, Prettier check, typecheck, and tests on PRs and pushes to `main`; enable branch protection requiring it to pass before merge (design Decision 13)
...
- [ ] 5.4 Build sign-in/sign-out UI, including messaging that Phase 1 data is local to the browser (not a security boundary)
...
- [ ] 7.6 Build entry entry/edit UI for assets, liabilities, income, and expenses
...
- [ ] 8.4 Build category breakdown chart/view
...
- [ ] 10.1 Build Settings/Account page shell
...
- [ ] 11.1 Set up base navigation/screen structure (Sign-in, Dashboard, Categories, Entries, Reports, Settings) as thin route files over feature screens
- [ ] 11.2 Configure app icons/branding placeholders and basic app shell
