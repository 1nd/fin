...
- Google Sign-In, real identity, or per-account partitions (change (B)) — this change uses a placeholder `userId`.
- Any finance domain model (accounts, snapshots, transactions, currency) — changes (C)+.
- Backup/export ((D)), Light/colorblind themes, live rate APIs, offline/PWA packaging.
...
Layer 2 defines a `SettingsRepository` (later changes add their own ports); layer 4 implements it on IndexedDB using `idb` (thin promise wrapper, no schema framework lock-in). Schema conventions set now, load-bearing for (B) and (D):
...
- `userId` is always a parameter flowing through the port — never read from a global — so swapping the placeholder for the Google id in (B) touches only the identity provider, not storage code.
...
- [Placeholder `userId` could leak into records that survive into the multi-account era] → Only settings exist before (B), and `plan.md` accepts settings living under a placeholder; the constant is defined once so (B) can find every use.
- [IndexedDB data is evictable even after `persist()`] → Accepted for this change; real mitigation is backup ((D)), which is why it's early in the roadmap.
